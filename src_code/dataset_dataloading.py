...
def _normalize_array(x: np.ndarray) -> np.ndarray:
    ...
    # add a small constant in the normalization to avoid division by 0 error
    return (x - min_val) / (max_val - min_val + 10**-100)
...
class WMHDataset(Dataset):
    def __init__(
        ...
        """
        A WMHDataset is a custom PyTorch dataset built for use with the WMH challenge.
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, torch.Tensor]:
        # We have a list of tuples, so every entry will return a (Path, Path, Path, int).
        # So first Path is t1, second is t2 and so on.
        ...
        # But, we only want the single slice, so we set the last index 'z' to 1.
...
class WMHPrefetchDataset(Dataset):
    def __init__(
        ...
        """
        A WMHPrefetchDataset is a custom PyTorch dataset built for use with the WMH challenge.
